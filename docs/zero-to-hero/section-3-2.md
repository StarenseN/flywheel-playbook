...
**Usage:** Run on any user-facing text (README, docs, UI copy, CLI help, error messages) before release. The "no regex, no scripts" instruction forces the model to read each sentence individually rather than pattern-matching. Pass 1 catches the line-level tells: em dashes as dramatic pauses, "It's not X, it's Y", "Here's why", "It's worth noting that", "Let's dive in". Pass 2 catches what only shows up across the whole document: an outline too tidy to be written, one point restated three ways, uniform sentence rhythm, a draft where nothing ever goes wrong. Run Pass 2 in a fresh session -- an agent auditing its own output in the same context defends its choices instead of reading them. Run with Claude (strong prose quality) or Gemini (catches different patterns than Claude). One caveat worth stating to whoever asks for this: the protocol makes text pass a human reader who knows the patterns, not a statistical AI detector, which scores the generating model's token fingerprint rather than surface style.
